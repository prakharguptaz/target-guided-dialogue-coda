...
def get_usefuljson_lines(inp_file):
    lines = []
    with jsonlines.open(inp_file) as reader:
        for obj in reader:
            if 0.2<obj['overlap_tr']<0.7 and 'State' in obj['da_string'] and obj['is_targetatend'] is True:
                lines.append(obj)
    return lines
...
def get_verbnouns(document):
    ...
    newfeatures_verb = []
    for f in features_verb:
        fwords = f.split()
        newf = []
        for fw in fwords:
            if "'" in fw and len(fw)<4:
                continue
            newf.append(fw)
        newf = ' '.join(newf)
        newfeatures_verb.append(newf)
    features_verb = newfeatures_verb
...
rel2text_path = '../../Commonsense-Path-Generator/learning-generator/utils/relation2text.json'
...
# pretrain_generator_ckpt = "./commonsense-path-generator.ckpt"
# pretrain_generator_ckpt = "./learning-generator/checkpoints_prefix_org/model.ckpt"
# checkpoints_prefix_org needs input_len to be 16, gpt.max_len to be 31
# pretrain_generator_ckpt = "./learning-generator/checkpoints_6len_wcontains_all-ents/model.ckpt"
pretrain_generator_ckpt = "../../Commonsense-Path-Generator/learning-generator/checkpoints_6lendict_wcontains/model.ckpt"
...
def augment_datapoint_otters(data, write_file, verbose=False, remove_overlap=False, split_entities_into_multi=False):
    ...
    with open(write_file, 'w') as outfile:
        ottersdata = []
        for dindex, datarow in enumerate(tqdm(data)):
            index = dindex#datarow[0]
            context = datarow['context'][-2:]#datarow[1].strip()
            # context_words = context.split()
            # if len(context_words) > 250 - 100:
            #     context = ' '.join(context_words[-150:])
            target = datarow['target']#datarow[2].strip()
            output = datarow['response']#datarow[3].strip()
            ...
            if len(response_keywords)==0 or len(target_keywords)==0:
                continue
            extra_keywords = list(set(response_keywords)- set(target_keywords))
            if len(extra_keywords)==0:
                continue
            
            context_index = len(context)-1
            context_keywords  = []
            while context_index>=0 and len(context_keywords)<2:
                csent = context[context_index]
                context_keywords += get_verbnouns(csent)
                context_index-=1
                # print(csent, context_keywords, context_index, context)
...
# infile = '../naacl-2021-fudge-controlled-generation/ottersdata/simple/in_domain_train_out.csv'
infile = 'data_onlynextturn/train_agendaseqwsrl.txt'
start_index = 17000
end_index = 18500
# data= get_csv_lines(infile)
data= get_usefuljson_lines(infile)

print("--- total data size = ", len(data))
data = data[start_index:end_index]
write_file = 'pathsdata_onlynextturn/augfp4_'+ infile.split('/')[-1]+'_'+str(start_index)+'-'+str(end_index)+outfile_suffix+'.jsonl'
